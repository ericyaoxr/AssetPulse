...
router.get("/", (req, res) => {
  const items = db.prepare("SELECT name FROM categories WHERE user_id = ? ORDER BY name").all(req.userId)
...
router.delete("/:name", (req, res) => {
  db.prepare("DELETE FROM categories WHERE user_id = ? AND name = ?").run(req.userId, req.params.name)
  res.json({ ok: true })
})
