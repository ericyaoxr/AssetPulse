      ...
      <div className="w-full max-w-sm space-y-8">
        <div className="flex flex-col items-center gap-3">
          ...
          <div className="text-center">
            <h1 className="font-mono text-2xl font-bold tracking-tight text-content-primary">
              Asset<span className="text-accent">Pulse</span>
            </h1>
            <p className="mt-1 text-sm text-content-muted">资产日均成本追踪</p>
          </div>
        </div>

        <div className="rounded-xl border border-edge bg-surface backdrop-blur-md p-6">
          <h2 className="mb-5 text-lg font-semibold text-content-primary">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm text-content-secondary mb-1">密码</label>
          ...
          <div className="mt-4 text-center text-sm text-content-muted">
            {mode === "login" ? (
              <>还没有账户？<button onClick={switchMode} className="text-accent hover:text-emerald-300">注册</button></>
            ) : (
              <>已有账户？<button onClick={switchMode} className="text-accent hover:text-emerald-300">登录</button></>
            )}
          </div>
        </div>
