  ...
  if (trash.length === 0) {
    return (
      <div className="space-y-5">
        <h1 className="text-2xl font-bold text-white">回收站</h1>
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white">回收站</h1>
          <p className="mt-1 text-sm text-white/50">{trash.length} 件已删除资产</p>
        </div>
        <button
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="flex-1 min-w-0">
                <h3 className="truncate text-white font-medium">{item.asset.name}</h3>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  ...
                  className="flex items-center gap-1.5 rounded-lg border border-emerald-500/30 px-3 py-1.5 text-xs font-medium text-emerald-400 transition-colors hover:bg-emerald-500/10"
                >
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-80 rounded-xl border border-white/10 bg-[#0D1B1E] p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-white/60">确定要永久删除所有资产吗？此操作不可撤销。</p>
            <div className="mt-4 flex gap-3">
              ...
              <button
                ...
                className="flex-1 rounded-lg border border-white/10 px-4 py-2 text-sm font-medium text-white/70 transition-colors hover:bg-white/5"
              >
                取消
              </button>
            </div>
      ...
      {confirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="w-80 rounded-xl border border-white/10 bg-[#0D1B1E] p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-white/60">确定要永久删除此资产吗？此操作不可撤销。</p>
            <div className="mt-4 flex gap-3">
              <button
